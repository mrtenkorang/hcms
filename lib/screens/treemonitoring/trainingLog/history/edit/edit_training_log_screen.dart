...
import 'package:hcms_revived2/screens/treemonitoring/trainingLog/history/edit/edit_training_log_controller.dart';
import 'package:hcms_revived2/utils/widgets/textFields/generic_text_field.dart';

class EditTrainingLogHistoryScreen extends StatefulWidget {
  const EditTrainingLogHistoryScreen({Key? key, required this.trainingLog}) : super(key: key);

  final TrainingLogModel trainingLog;

  @override
  State<EditTrainingLogHistoryScreen> createState() => _EditTrainingLogHistoryScreenState();
}

class _EditTrainingLogHistoryScreenState extends State<EditTrainingLogHistoryScreen> {
  final EditTrainingLogController controller = Get.put(EditTrainingLogController());

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.initializeData(widget.trainingLog);
    });
  }

  @override
  Widget build(BuildContext context) {
    return GetBuilder<EditTrainingLogController>(
      builder: (controller) => _TrainingLogView(controller: controller, trainingLog: widget.trainingLog),
    );
  }
}

class _TrainingLogView extends StatelessWidget {
  final EditTrainingLogController controller;
  final TrainingLogModel trainingLog;

  const _TrainingLogView({Key? key, required this.controller, required this.trainingLog}) : super(key: key);
              ...
              Expanded(
                child: controller.isLoadingFarmers.value
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = controller.selectedParticipant.any((p) => p.id == farmer.id);
                    return ListTile(
                      contentPadding: const EdgeInsets.symmetric(
                        horizontal: 16,
                        vertical: 12,
                      ...
                      onTap: () {
                        if (isSelected) {
                          controller.selectedParticipant.remove(farmer);
                        } else {
      ...
      children: [
        Row(
          children: [
            ...
            const Spacer(),
            if (controller.selectedParticipant.isNotEmpty)
              IconButton(
          ...
          rows: controller.selectedParticipant.map((farmer) {
            ...
            return DataRow(
              ...
              cells: [
                ...
                DataCell(Text(farmer.contact ?? 'N/A')),
                DataCell(Text(farmer.communityId?.toString() ?? 'N/A')),
              ],
            );
          }).toList(),
        ),
      ...
      children: [
        Expanded(
          child: _buildActionButton(
            ...
            icon: Icons.save,
            onPressed: () {
              controller.updateTrainingLogOffline();
            },
        ...
        Expanded(
          child: _buildActionButton(
            ...
            onPressed: () {
              controller.submitUpdatedTrainingLog();
            },
  ...
  Widget _buildFarmerDropdown() {
    return Container(
      ...
      child: Obx(() {
        return ListTile(
          title: Text(
            'Select farmers to add participants',
            style: TextStyle(
              color: Colors.grey[600],
              fontSize: 16,
            ),
          ),
          subtitle: controller.selectedParticipant.isNotEmpty
              ? Text('${controller.selectedParticipant.length} participants selected')
              : null,
